    ...
    return (
      <main className="min-h-screen bg-gray-50">
        <div className="max-w-md mx-auto px-6 py-16 text-center">
          ...
          <p className="text-gray-500 text-sm">{t('flashcards.noOnboarding')}</p>
        </div>
      </main>
    )
  }
  ...
  if (cards.length === 0) {
    return (
      <main className="min-h-screen bg-gray-50">
        <div className="max-w-md mx-auto px-6 py-16 text-center">
          <div className="text-4xl mb-4">🃏</div>
          <h1 className="text-xl font-semibold text-gray-900 mb-2">{t('flashcards.noCards')}</h1>
          <p className="text-gray-500 text-sm">{t('flashcards.noCardsText')}</p>
        </div>
      </main>
    )
  }
    ...
    return (
      <main className="min-h-screen bg-gray-50">
        <div className="max-w-md mx-auto px-6 py-16 text-center">
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mb-2">{t('flashcards.sessionDone')}</h1>
          <p className="text-gray-500 mb-8">{t('flashcards.sessionDoneText', { count: cards.length })}</p>
          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="bg-green-50 rounded-2xl p-4">
              <p className="text-2xl font-bold text-green-600">{correct}</p>
              <p className="text-xs text-green-600 mt-1">{t('flashcards.known')}</p>
            </div>
            <div className="bg-yellow-50 rounded-2xl p-4">
              <p className="text-2xl font-bold text-yellow-600">{doubt}</p>
              <p className="text-xs text-yellow-600 mt-1">{t('flashcards.doubt')}</p>
            </div>
            <div className="bg-red-50 rounded-2xl p-4">
              <p className="text-2xl font-bold text-red-600">{wrong}</p>
              <p className="text-xs text-red-600 mt-1">{t('flashcards.wrong')}</p>
            </div>
          </div>
          <button
            onClick={restart}
        ...
        >
          {!flipped ? (
            <div className="text-center">
              <p className="text-xs text-gray-400 mb-4 uppercase tracking-wide">{t('flashcards.question')}</p>
              <p className="text-lg font-semibold text-gray-900">{currentCard.question}</p>
              <p className="text-xs text-gray-400 mt-6">{t('flashcards.clickToReveal')}</p>
            </div>
          ) : (
            <div className="text-center">
              <p className="text-xs text-gray-400 mb-4 uppercase tracking-wide">{t('flashcards.answer')}</p>
